...
export function useMultichainCurrencyBalance(
    ...
    const getBalance = useCallback(() => {
        ...
        const addr = isTo ? currency.address : FCL_TO_MUMBAI[currency.symbol]
        let contract = new web3.eth.Contract(ERC20_ABI as any, addr)
        contract.methods
            .balanceOf(account)
            .call()
            .then((response) => {
                // const amount = CurrencyAmount.fromRawAmount(currency, response || 0)
                const amount = web3.utils.fromWei(response || 0)
                setValue(amount)
            })
            .catch((ex) => {
                console.error(ex)
    ...
    }, [account, chainId, currency, getBalance])
