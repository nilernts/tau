...
export async function getLendMarkets(account: string): Promise<any[]> {
    ...
    const contract = new web3.eth.Contract(TauVault.abi as any, DEPLOYMENTS.vault.TauVault)
    ...
    for (let i = 0; i < +marketsLength.toString(); i++) {
        ...
        const liquidity = await getTokenBalance(DEPLOYMENTS.vault.TauVault, asset)
