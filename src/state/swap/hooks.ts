...
export function useSwapActionHandlers(): {
    onCurrencySelection: (field: Field, currency: IFCL_TOKEN) => void
    onSwitchTokens: () => void
    ...
    const onCurrencySelection = useCallback(
        (field: Field, currency: IFCL_TOKEN) => {
            dispatch(
                selectCurrency({
                    field,
...
export function useDerivedSwapInfo(doArcher = false): {
    currencies: { [field in Field]?: IFCL_TOKEN }
    currencyBalances: { [field in Field]?: number } // CurrencyAmount<Currency> }
    parsedAmount: string | undefined // CurrencyAmount<Currency> | undefined
    inputError?: string
    ...
    const inputCurrency = FCL_TOKENS.find((f) => f.id === inputCurrencyId) // useCurrency(inputCurrencyId)
    const outputCurrency = FCL_TOKENS.find((f) => f.id === outputCurrencyId) // useCurrency(outputCurrencyId)
    const recipientLookup = useENS(undefined)
    const to: string | null = (recipient === null ? account : recipientLookup.address) ?? null

    // console.error("TODODODODOD: FETCH TOKEN BALANCES IN state/swap/hooks.ts")
    ...
    const parsedAmount = typedValue // tryParseAmount(typedValue, (isExactIn ? inputCurrency : outputCurrency) ?? undefined)

    const bestTradeExactIn = undefined // useTradeExactIn(isExactIn ? parsedAmount : undefined, outputCurrency ?? undefined, {
    //     maxHops: singleHopOnly ? 1 : undefined,
    // })

    const bestTradeExactOut = undefined // useTradeExactOut(inputCurrency ?? undefined, !isExactIn ? parsedAmount : undefined, {
    // maxHops: singleHopOnly ? 1 : undefined,
    // })
    ...
    const currencyBalances = {
        [Field.INPUT]: 6900.69, // relevantTokenBalances[0],
        [Field.OUTPUT]: 4200.23, // relevantTokenBalances[1],
    }

    const currencies: { [field in Field]?: IFCL_TOKEN } = {
    ...
    const [_, setUserGasEstimate] = useUserArcherGasEstimate()
    ...
    useEffect(() => {
        ...
        //                 .estimateGas(tx)
        //                 .then((gasEstimate) => {
        //                     console.error("Estimated gas!!:", { call, gasEstimate })
        //                     return {
        //                         call,
        //                         gasEstimate,
        //                     }
    ...
    }, [doArcher, v2Trade, userTipManualOverride, library, setUserGasEstimate])
