...
export function useDerivedMintInfo(
    currencyA: IFCL_TOKEN | undefined,
    currencyB: IFCL_TOKEN | undefined
): {
    ...
    // balances
    const balances = useFlowTokenBalances([currencyA?.symbol, currencyB?.symbol], fcl.account)
    // const balances = useCurrencyBalances(account ?? undefined, [
    //     currencies[Field.CURRENCY_A],
    //     currencies[Field.CURRENCY_B],
    // ])
    const currencyBalances: { [field in Field]?: string } = {
        [Field.CURRENCY_A]: balances?.[0] ?? "0.0",
        [Field.CURRENCY_B]: balances?.[1] ?? "0.0",
    }
