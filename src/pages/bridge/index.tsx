...
import { FCL_TOKENS, FCL_TO_MUMBAI, IFCL_TOKEN } from "../../fcl/addresses/addresses"
import { useBridgeDeposit } from "../../fcl/transactions/bridge/deposit"
...
export default function Bridge() {
    ...
    const availableChains = [ChainId.XDC_APOTHEM, ChainId.MATIC_TESTNET]
    ...
    const currentChainFrom = { id: chainId, icon: "/images/networks/flow-network.svg", name: "Flow Network" }
    ...
    const [tokenList, setTokenList] = useState<IFCL_TOKEN[] | null>([])
    const [currency0, setCurrency0] = useState<IFCL_TOKEN | null>(null)
    const [currencyAmount, setCurrencyAmount] = useState<string | null>("")
    const [tokenToBridge, setTokenToBridge] = useState<IFCL_TOKEN | null>(null)
    ...
    const [approvalState, approve] = useApproveCallback(typedDepositValue, DEPLOYMENTS.bridge.Bridge_Apothem)
    ...
    useEffect(() => {
        const meTokens = ["DCT", "MAV", "PRT"]

        let tokens = meTokens.map((r) => {
            return FCL_TOKENS.find((t) => t.symbol === r)
        })
    ...
    const handleChainFrom = useCallback(
        (chain: Chain) => {
            ...
            if (changeTo.id !== ChainId.XDC_APOTHEM && chain.id !== ChainId.XDC_APOTHEM) {
    ...
    const handleChainTo = useCallback(
        (chain: Chain) => {
            ...
            if (changeFrom.id !== ChainId.XDC_APOTHEM && chain.id !== ChainId.XDC_APOTHEM) {
    ...
    const insufficientBalance = () => {
        if (currencyAmount && selectedCurrencyBalance) {
            try {
                const balance = parseFloat(selectedCurrencyBalance.toFixed(3))
    ...
    const bridgeToken = async () => {
        ...
        console.debug("Token to bridge:", tokenToBridge, currencyAmount)

        const token = tokenToBridge
        const amountToBridge = currencyAmount
        ...
        try {
            ...
            const res = await useBridgeDeposit(fcl, token.symbol, amountToBridge, data)
            console.debug("Flow result:", res)
            const withdrawResult = await withdraw(
                activeAccount,
                FCL_TO_MUMBAI[token.symbol],
                ethers.utils.parseUnits(amountToBridge, 18).toString(),
                data
            )
            console.debug("Withdraw Result:", withdrawResult)

            setShowConfirmation(false)
            addTransaction(
                { hash: withdrawResult.transactionHash },
                {
                    summary: `${`Bridge `} ${token.symbol}`,
                }
            )
            setLoading(false)
        } catch (err) {
                        ...
                        <div className="p-4 text-center">
                            <div className="justify-between space-x-3 items-center">
                                <Typography component="h3" variant="base">
                                    {`Bridge tokens to and from the Flow Network`}
                                </Typography>
                            </div>
                        </div>
