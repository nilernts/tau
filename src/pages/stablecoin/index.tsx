...
export default function Vault(): JSX.Element {
    ...
    const taUSDBalance = useTokenBalance(DEPLOYMENTS.stablecoin.taUSD)
    console.debug("Tausd balance:", taUSDBalance.value.toString())
    ...
    return (
        <>
            <Head>
                <title>Stablecoin | Tau</title>
                <meta key="description" name="description" content="Tau Stablecoin" />
            </Head>
                                    ...
                                    <div className={`col-span-12 md:col-span-3 space-y-4`}>
                                        <div className={`hidden md:block`}>
                                            <div className={`col-span-12 md:col-span-4 bg-dark-800 px-6 py-4 rounded`}>
                                                <div className="mb-2 text-2xl text-emphesis">{`Tau Stablecoin (taUSD)`}</div>
                                                <div className="mb-4 text-base text-secondary">
                                                    <p>
                                                        {`taUSD is an`} <strong>overcollateralized</strong>,{" "}
                                                    ...
                                                    <p className="mt-2">&nbsp; &nbsp;</p>
                                                    <p className="mt-2">
                                                        {`Lenders on Tau can collateralize their deposits (ibTokens) to borrow taUSD which they can use inside/outside Tau
                                                ...
                                                >
                                                    <div className="flex items-center text-center justify-between py-2 text-emphasis">
                                                        taUSD Balance:{" "}
                                                        {formatNumberScale(
                                                            fromDecimals(taUSDBalance?.value.toString() ?? "0"),
                                                            true,
                                                            2
                                                        )}
                                                    </div>
