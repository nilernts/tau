...
function Balance({ balance }: { balance: string }) {
    return (
        <div className="whitespace-nowrap overflow-hidden max-w-[5rem] overflow-ellipsis" title={balance}>
            {formatNumberScale(Number(balance).toFixed(4), false, 2)}
        </div>
    )
}
...
function CurrencyRow({
    ...
    const fcl = useFCL()
    const balance = useFlowTokenBalances([currency.symbol], fcl.account)?.[0]?.balance
...
export default function CurrencyList({
    ...
    const itemData: IFCL_TOKEN[] = useMemo(() => {
        return currencies
    }, [currencies])
    ...
    const Row = useCallback(
        function TokenRow({ data, index, style }) {
            ...
            const isSelected = Boolean(currency && selectedCurrency && selectedCurrency.symbol === currency.symbol)
