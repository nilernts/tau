...
import { TAU_DISTRIBUTOR_ADDRESS, MINICHEF_ADDRESS } from "../../constants/addresses"
...
const FarmListItem = ({ farm }) => {
    ...
    const [approvalState, approve] = useApproveCallback(typedDepositValue, TAU_DISTRIBUTOR_ADDRESS[chainId])
                                        ...
                                        if (!balance.equalTo(ZERO)) {
                                            if (liquidityToken?.symbol == "SOLAR") {
                                                try {
                                                    ...
                                                    const newValue =
                                                        parseFloat(balance.toFixed(liquidityToken?.decimals)) - minValue
                        ...
                        {account && (
                            <div className="pr-4 mb-2 text-left cursor-pointer text-secondary">
                                ...
                                {farm.lpPrice && amount
                                    ? ` (` +
                                      formatNumberScale(
                                          farm.lpPrice * Number(amount?.toSignificant(18) ?? 0),
                                          true,
                                          2
                                      ) +
                                      `)`
