...
import { useTauDistributorContract, useBNBPairContract, useSolarMovrContract, useTauVaultContract } from "../../hooks"
...
export function useUserInfo(farm, token) {
    ...
    const contract = useTauVaultContract()
...
export function useSolarPositions(contract?: Contract | null) {
    ...
    return useMemo(() => {
        if (!pendingTau || !userInfo || !userLockedUntil) {
            return []
        }
        return zip(pendingTau, userInfo, userLockedUntil)
            .map((data, i) => ({
                id: args[i][0],
    ...
    }, [args, pendingTau, userInfo, userLockedUntil])
}

export function usePositions() {
    return useSolarPositions(useTauVaultContract())
}
...
export function useVaults() {
    return useSolarVaults(useTauVaultContract())
}
...
export function useSolarVaultInfo() {
    return useVaultInfo(useTauVaultContract())
}
