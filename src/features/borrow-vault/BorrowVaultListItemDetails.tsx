...
import { useFCLBorrow } from "../../fcl/transactions/vault/borrow"

const BorrowVaultListItemDetails = ({ farm, fcl }) => {
    ...
    const borrowBalance = +farm.borrows

    const typedDepositValue = depositValue // tryParseAmount(depositValue, liquidityToken)
    const typedWithdrawValue = repayValue // tryParseAmount(repayValue, liquidityToken)
                        ...
                        <div className="col-span-2 text-center md:col-span-1">
                            ...
                            <div className="relative flex items-center w-full mb-4">
                                ...
                                {fcl.account && (
                                    <Button
                                        ...
                                        onClick={() => {
                                            if (maxBorrow !== 0) {
                                                setDepositValue(maxBorrow.toFixed(2))
                                            }
                                        }}
                            ...
                            <Button
                                ...
                                onClick={async () => {
                                    setPendingTx(true)
                                    try {
                                        const tx = await useFCLBorrow(fcl, farm.symbol, typedDepositValue)

                                        addTransaction(
                                            { hash: tx.transactionHash, ...tx },
                                            {
                                                summary: `${`Borrow`} ${farm.symbol}`,
                                            }
                                        )
                                    } catch (error) {
                                        console.error(error)
                            ...
                            <div className="relative flex items-center w-full mb-4">
                                ...
                                {fcl.account && (
                                    <Button
                                        ...
                                        onClick={() => {
                                            if (borrowBalance !== 0) {
                                                setRepayValue(borrowBalance.toFixed(2))
                                            }
                                        }}
                                        className="absolute border-0 right-4 focus:ring focus:ring-light-purple"
                                    >
