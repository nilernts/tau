...
interface PositionCardProps {
    ...
    border?: string
    stakedBalance?: string // optional balance to indicate that liquidity is deposited in mining pool
}
...
export default function FullPositionCard({ pair, border, stakedBalance }: PositionCardProps) {
    ...
    const currency0 = FCL_TOKENS.find((t) => t.symbol === pair.token0)
    const currency1 = FCL_TOKENS.find((t) => t.symbol === pair.token1)
            ...
            >
                <div className="p-4 space-y-4">
                    <div className="px-4 py-4 space-y-1 text-sm rounded text-high-emphesis bg-dark-900">
                        ...
                        {stakedBalance && (
                            <div className="flex items-center justify-between">
                                <div>{`Total Pool tokens`}:</div>
                                <div className="font-semibold">
                                    {Number(stakedBalance).toFixed(3)} {"K"}
                                </div>
                            </div>
                        )}
                        {/* <div className="flex items-center justify-between">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <Button
                            ...
                            variant="link"
                            onClick={() => {
                                updateAddTokensRouter(currency0.id, currency1.id)
