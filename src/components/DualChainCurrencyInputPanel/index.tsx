...
import { FCL_TO_MUMBAI, IFCL_TOKEN } from "../../fcl/addresses/addresses"
...
export default function DualChainCurrencyInputPanel({
    ...
    const [toCurrency, setToCurrency] = useState<IFCL_TOKEN | null>(null)
    ...
    //     currency ?? undefined
    // )
    const selectedCurrencyBalance = useFlowTokenBalances([currency?.symbol], fcl.account)
    ...
    useEffect(() => {
        ...
        let contractAddr = FCL_TO_MUMBAI[currency.symbol]
                    ...
                    >
                        <>
                            {selectedCurrencyBalance && (
                                <Button
                                    onClick={() => {
                                        onMax(Number(selectedCurrencyBalance?.[0]?.balance).toFixed(4))
                                    }}
                                    size="xs"
                                ...
                                <div className="flex flex-col">
                                    ...
                                    >
                                        <>
                                            {`Balance:`}{" "}
                                            {formatNumberScale(
                                                Number(selectedCurrencyBalance?.[0]?.balance).toFixed(4)
                                            )}{" "}
                                            {currency.symbol}
                                        </>
                                    </div>
            ...
            >
                <div className="flex flex-row justify-between flex-1">
                    ...
                    <div className={classNames("flex items-center w-full px-0")}>
                        <div className="text-xs font-medium text-secondary whitespace-nowrap">
                            {formatNumber(Number(selectedCurrencyBalanceDest)?.toFixed(2))} {currency?.symbol}
                        </div>
                    </div>
                </div>
            </div>
