...
const builders = {
    ...
    xdc_apothem: (chainName: string, data: string, type: "transaction" | "token" | "address" | "block") => {
        const prefix = "https://apothem.blocksscan.io"
        switch (type) {
            case "transaction":
                return `${prefix}/tx/${data}`
            case "address":
...
const chains: ChainObject = {
    ...
    [ChainId.XDC_APOTHEM]: {
        chainName: "xdc_apothem",
        builder: builders.xdc_apothem,
    },
    [ChainId.MATIC]: {
        chainName: "mainnet",
