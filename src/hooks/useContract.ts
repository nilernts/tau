...
import TAU_DISTRIBUTOR_ABI from "../constants/abis/solar-distributor.json"
...
import TauFactoryABI from "../constants/artifacts/contracts/swap/TauFactory.sol/TauFactory.json"
import TauRouter02ABI from "../constants/artifacts/contracts/swap/TauRouter02.sol/TauRouter02.json"
...
export function useTauDistributorContract(withSignerIfPossible?: boolean): Contract | null {
    const { chainId } = useActiveWeb3React()
    return useContract(chainId && TAU_DISTRIBUTOR_ADDRESS[chainId], TauDistributor.abi, withSignerIfPossible)
}

export function useTauVaultContract(withSignerIfPossible?: boolean): Contract | null {
    const { chainId } = useActiveWeb3React()
    return useContract(chainId && TAU_VAULT_ADDRESS[chainId], SOLAR_VAULT_ABI, withSignerIfPossible)
}
...
export function useSwapFactoryContract(withSignerIfPossible?: boolean): Contract | null {
    const { chainId } = useActiveWeb3React()
    return useContract(chainId && FACTORY_ADDRESS[chainId], TauFactoryABI.abi, withSignerIfPossible)
}
...
export function useRouterContract(useArcher = false, withSignerIfPossible?: boolean): Contract | null {
    ...
    const abi = useArcher ? ARCHER_ROUTER_ABI : TauRouter02ABI.abi
